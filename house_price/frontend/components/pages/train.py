...
def render_train_page(client: FastAPIClient, model_type: str):
    """
    ...
    if method == "Données manuelles":
        ...
        if st.button("Lancer l'entraînement"):
            ...
            metrics = client.train_model(x, y, model_type=model_type)
        ...
        if uploaded_file and st.button("Lancer l'entraînement"):
            metrics = client.train_model_file(file_path=uploaded_file, model_type=model_type)
            st.success(f"Entraînement terminé ! Metrics : {metrics}")
