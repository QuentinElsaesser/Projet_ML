...
async def train_model(request: Request, data: TrainRequest) -> Dict[str, Any]:
    ...
    Args:
        request (Request): Instance FastAPI pour accéder au modèle via `app.state.model_manager`
...
async def evaluate_model(request: Request, data: EvaluateRequest) -> Dict[str, Any]:
    ...
    x = np.array(data.x)
    y = np.array(data.y)
    metrics = manager.evaluate_model(x, y)
    return {"metrics": metrics}